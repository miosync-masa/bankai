...
logger = logging.getLogger("bankai.structures.md_features")
...
class MDFeaturesGPU(GPUBackend):
    ...
    def _calculate_rmsd_gpu(
        self, coords_batch: "cp.ndarray", ref_coords: "cp.ndarray"
    ) -> "cp.ndarray":
        """GPU版RMSD計算"""
    ...
    def _calculate_contacts_batched(
        self, trajectory: np.ndarray, backbone_indices: np.ndarray, batch_size: int
    ) -> np.ndarray:
    ...
    def _count_contacts_gpu(self, coords_batch: "cp.ndarray") -> "cp.ndarray":
        ...
        for i in range(n_frames):
            # 距離行列計算
            distances_sq = cp.sum(
                (coords_batch[i, :, None, :] - coords_batch[i, None, :, :]) ** 2, axis=2
            )
    ...
    def _calculate_dihedrals_batched(
        self, trajectory: np.ndarray, backbone_indices: np.ndarray, batch_size: int
    ) -> np.ndarray:
        """バッチ処理で二面角計算"""
        n_frames = trajectory.shape[0]
...
def calculate_rmsd_gpu(
    coords1: NDArray, coords2: NDArray, backend: Optional[GPUBackend] = None
) -> float:
    """RMSD計算のスタンドアロン関数"""
    if backend is None:
        backend = GPUBackend()
...
def calculate_radius_of_gyration_gpu(
    coords: NDArray, backend: Optional[GPUBackend] = None
) -> float:
    """Rg計算のスタンドアロン関数"""
    if backend is None:
        backend = GPUBackend()
...
def calculate_contacts_gpu(
    coords: NDArray, cutoff: float = 8.0, backend: Optional[GPUBackend] = None
) -> int:
    """接触数計算のスタンドアロン関数"""
    if backend is None:
        backend = GPUBackend()
