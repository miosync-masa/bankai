...
logger = logging.getLogger("bankai.residue.causality")
...
class CausalityAnalyzerGPU(GPUBackend):
    ...
    def _calculate_structural_causality_generic(
        self,
    ...
    def _calculate_transfer_entropy_exact(
        self,
    ...
    def _discretize_signal(
        self, signal: Union[np.ndarray, cp.ndarray], n_bins: int
    ) -> Union[np.ndarray, cp.ndarray]:
        """信号を離散化"""
        # パーセンタイルベースのビニング
    ...
    def compute_lagged_correlation(
        self, series_i: np.ndarray, series_j: np.ndarray, max_lag: Optional[int] = None
        ...
        for res_i in residues_i:
            for res_j in residues_j:
                ...
                result = self.calculate_structural_causality(
                    anomaly_scores[res_i], anomaly_scores[res_j]
                )
    ...
    def compute_granger_causality(
        self, series_i: np.ndarray, series_j: np.ndarray, order: int = 5
    ) -> float:
        """
        Granger因果性を計算（改良版）
        ...
        for i in range(order):
            xy_lagged[:, order + i] = (
                x_gpu[order - i - 1 : -i - 1] if i > 0 else x_gpu[order - 1 : -1]
            )
...
def calculate_structural_causality_gpu(
    anomaly_i: np.ndarray,
    anomaly_j: np.ndarray,
    lag_window: int = 200,
    event_threshold: float = 1.0,
...
def compute_lagged_correlation_gpu(
    series_i: np.ndarray,
    series_j: np.ndarray,
    max_lag: int = 200,
...
def detect_causal_pairs_gpu(
    anomaly_scores: dict[int, np.ndarray], threshold: float = 0.2, **kwargs
) -> list[CausalityResult]:
    """因果ペア検出のスタンドアロン関数"""
...
def compute_transfer_entropy_gpu(
    ...
    analyzer = CausalityAnalyzerGPU(history_length=history_length, n_bins=n_bins)
    if backend:
