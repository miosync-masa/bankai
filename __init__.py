...
class _GPUEnvironment:
    ...
    __slots__ = (
        'has_cupy', 'gpu_available', 'gpu_name',
        'gpu_memory', 'cuda_version', 'compute_capability',
    )
    ...
    def _detect(self):
        try:
            ...
            if cp.cuda.runtime.getDeviceCount() > 0:
                ...
                logger.info(f"GPU detected: {self.gpu_name} ({self.gpu_memory:.1f} GB, CUDA {self.cuda_version})")
            else:
...
def __getattr__(name: str):
    ...
    if name == 'perform_two_stage_analysis_gpu':
        from bankai.analysis.two_stage_analyzer_gpu import perform_two_stage_analysis_gpu
    ...
    if name in _result_types:
        from bankai import types as _types
